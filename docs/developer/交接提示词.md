...
- **技术栈**：Python 3.10+、pandas、ccxt（可选）、MCP 协议
...
- **单元测试**：10 个文件，全部通过
- **集成测试**：2 个文件，全部通过
- **测试套件**：`tests/run_tests.py` 支持 `unit/email/integration/stress/all`，并支持 `--list`、`--file`
...
1. **阅读本文档**，理解项目现状
2. **运行测试**：`python tests/run_tests.py unit` 验证环境
