...
- `MCP_FORCE_PRINT_TO_STDERR=True|False`（默认 True；将 print() 导向 stderr，避免 stdout 污染）
- `MCP_REDIRECT_STDOUT=True|False`（默认 False；高风险：可能破坏 MCP stdio 协议，仅用于排障）
- MCP 调用审计与备份（可选）：
  - `MCP_CALL_LOG_ENABLED=True|False`
  - `MCP_CALL_LOG_INCLUDE_ARGS=True|False`
  - `MCP_CALL_LOG_MAX_VALUE_CHARS`
  - `MCP_CALL_LOG_MAX_RESULT_CHARS`
  - `MCP_CALL_BACKUP_ENABLED=True|False`
  - `MCP_CALL_BACKUP_DIR`
  - `MCP_CALL_BACKUP_MAX_RESULT_CHARS`
  - `MCP_CALL_BACKUP_INCLUDE_TRACEBACK=True|False`

### 2.3 MCP Tools 开关（软禁用）
...
- 队列/结果键：
  ...
  - `NOTIFY_DEFAULT`（默认 `serverchan`，逗号分隔）
  - `NOTIFY_FAILOVER`（默认 `serverchan`，逗号分隔）
