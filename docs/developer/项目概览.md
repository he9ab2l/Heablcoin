...
## 使用与测试
- **MCP 客户端**：在 Claude/Windsurf 配置指向 `python /absolute/path/to/your/Heablcoin.py`（绝对路径），并设置 UTF-8 环境。
- **终端测试**：`python tests/run_tests.py unit` 快速回归；`python tests/run_tests.py email` 验证 SMTP。
- **常用工具**：`ai_run_pipeline`（多阶段 AI）、`ai_call_role`/`ai_reason`/`ai_write` 等角色调用；市场分析与交易工具详见 `docs/developer/API参考.md`。
...
## 运行建议
- 开发：保持 `USE_TESTNET=True`，先跑 `python tests/run_tests.py unit`；如需验证交易所连通性（可能需要联网与交易所配置），运行 `python Heablcoin-test.py --quick`。
