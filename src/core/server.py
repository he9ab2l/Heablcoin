...
#
# 🧩 文件结构：
# - 依赖（标准库）：asyncio, builtins, csv, dataclasses, datetime, email, html, json, logging, os, pathlib, re ... 等 18 个
# - 依赖（第三方）：ccxt, dotenv, mcp, numpy, pandas
# - 依赖（本地）：core.cloud.task_executor, core.exchange, core.mcp_safety, skills.report.flexible_report.service, tools.admin_tools, tools.cloud_tools, tools.governance_tools, tools.learning_tools, tools.market_analysis_tools, tools.orchestration_tools, tools.personal_analytics_tools, tools.research_tools ... 等 16 个
#
# 🕒 创建时间：2025-12-19
############################################################
...
# --- src 分层目录支持 ---
# 核心实现位于 src/core，但运行入口可能来自仓库根目录的 Heablcoin.py，
# 此处做一次兜底初始化，确保可导入 core/tools/skills/utils/storage。
try:
...
from tools.personal_analytics_tools import register_tools as _register_personal_analytics_tools
...
def place_order(symbol: str, side: str, amount: float, price: float = None, order_type: str = "market") -> str:
    ...
    try:
        ...
        # 学习模块：下单纪律拦截（可选）
        try:
            from skills.learning.discipline import evaluate_order
