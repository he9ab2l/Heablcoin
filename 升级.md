...
--file test_integration_simple.py：运行快速冒烟测试，只执行简易集成测试，耗时短，适合开发中频繁验证
 GitHub
 。预期输出示例包括关键功能的通过情况
 GitHub
 。
...
**环境配置：** 部署前请确保安装正确的 Python 环境（>=3.10）以及项目依赖。按照安装指南，首先克隆仓库并进入目录，然后复制`.env.example`为`.env`文件:contentReference[oaicite:137]{index=137}。编辑 `.env` 填入所需配置项：
