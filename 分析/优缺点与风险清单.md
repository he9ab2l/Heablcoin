...
> 本文件为长期维护入口（无时间戳）。
>
> - 历史快照：`分析/优缺点与风险清单_20251219_010927.md`
...
- **证据**:
  - `src/core/cloud/api_manager.py` 中 `get_available_endpoints` 被定义了两次（Python 后定义覆盖前定义）
- **影响**:
...
- **机制**:
  ...
  - 开启后：
    - 冷静期锁定：`locked_until`
    - 可选趋势拦截：调用 `skills.market_analysis` 的 `technical` 输出
