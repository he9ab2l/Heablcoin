...
- `get_market_overview(mode)`
  ...
  - 为每币种算简单 buy/sell 信号
  - 输出：simple 为红绿灯概览；professional 需要继续精读后半段（本次读取到 simple 分支开始处）
...
> 现状观察：回调 payload 中未见签名（文档提到 `TASK_WEBHOOK_SECRET`/`X-Heabl-Signature`，需后续在其他模块确认是否实现）

## 5. 当前缺陷/风险点（基于已读代码的“证据化”结论）

- **[High] 任务回调未见签名**：`enhanced_publisher._invoke_callback` 直接 POST JSON，无鉴权头（证据：`enhanced_publisher.py` 331-339 行附近）。如果公网暴露 callback，存在伪造/重放风险
- **[Mid] `get_market_overview(professional)` 未完整验证**：本次读取截断到 simple 分支开始处，professional 输出细节需再读剩余代码（信息缺失）
- **[Mid] 风控覆盖不完全**：`place_order` 对单笔限额校验存在，但“每日限额”仅用于展示与统计，未在下单处强制拦截（证据：`place_order` 只检查 `_get_max_trade_amount`，未检查 `_get_daily_trade_limit`）
- **[Low] 交易数量/精度校验不明确**：`place_order` 直接使用 `amount` 下单，未见市场最小下单量/步进精度校验（需后续检查 `utils/validators.py` 与交易适配器是否覆盖）
...
- 输出：
  - `分析/工具清单索引_*.md`（把所有 MCP tool 形成可搜索目录）
  - `分析/差异分析_*.md`（后续基于 git diff/文件变更）
