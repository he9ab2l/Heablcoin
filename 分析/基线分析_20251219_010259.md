...
- 终端：`python tests/run_tests.py --quick` 或 `python Heablcoin-test.py --quick`
...
## 6. 项目输出/数据文件（用户可感知）
- 交易记录：`trade_history.csv`（同时存在 SQLite：`data/trades.db` 或 `data/trades.db` 类路径）
...
- 学习会话存储：位于 `skills/learning/storage.py` 管理的本地持久化（具体路径待后续精读确认）
...
- **可观测性导向**：smart logger / smart cache（可用性需进一步验证）
- **可扩展性**：多模型路由、存储适配器、API endpoint 管理

## 8. 缺陷/风险点（当前观察，后续需精读验证）

### 8.1 仓库状态风险
- 当前工作区 `git status` 显示大量 `M/A`（未提交变更），说明当前目录不是“干净基线”
- 本分析记录的是 HEAD，但实际运行代码可能已被修改（需用 `git diff` 对齐）
...
### 8.3 安全与误操作风险
- 交易相关能力存在，但实际风控细节（白名单/限额/下单前校验/测试网切换）需要进一步核查（目前仅看到 server 侧读取 env 并在文档声明）

### 8.4 任务/并发一致性
- `TaskExecutor` 为线程轮询模型，状态更新与重试逻辑依赖 `EnhancedCloudTaskPublisher` 的一致性实现
- 回调签名校验、幂等、失败重放等需要进一步核对实现文件（后续精读）

### 8.5 worker/Redis 依赖
- `qinglong_worker.py` 依赖 Redis + `Heablcoin` 内的 `send_email/get_exchange`，部署环境容易出现路径/依赖/凭证问题

## 9. 下一步只读分析计划（本分析之后）
- 精读 `src/core/server.py` 的后半部分：
  - 交易执行/风控工具列表
  - 账户/仓位/订单工具
  - 报告生成与落盘路径
...
- 形成 `分析/模块地图.md` 与 `分析/工具清单索引.md`

## 10. 差异分析机制（建议操作方式）
后续你只需要告诉我“做一次差异分析”，我会：
- 记录新的时间戳与 `git rev-parse HEAD`
- 输出 `git diff <旧基线>..<新基线>` 的摘要（只读）
- 把“新增/删除/改动模块、工具、配置项变化、风险影响”写入 `分析/差异分析_*.md`
