...
- 时间（北京时间）: `20251219_010759`
- 目标：用最短路径说明“如何安装/配置/启动/验证/排错”，并列出关键环境变量分类
...
### 1.3 配置 `.env`
- 从模板复制：`cp .env.example .env`（Windows 可 `copy .env.example .env`）
...
### 1.4 本地自检
- 快速：`python tests/run_tests.py --quick`
- 邮件链路：`python tests/run_tests.py email`（可能发送真实邮件）
...
- 验证：`python tests/run_tests.py --quick`
...
- **开发/验证**：先跑 `tests/run_tests.py --quick`
