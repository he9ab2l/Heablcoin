# Heablcoin 测试套件

完整的测试套件，用于验证所有优化功能和MCP工具。

## 📁 测试结构

```
tests/
├── __init__.py                    # 测试模块初始化
├── README.md                      # 本文档
├── run_tests.py                   # 统一测试入口 ⭐
│
├── 单元测试/
│   ├── test_smart_logger.py       # 智能日志系统测试
│   ├── test_smart_cache.py        # 智能缓存系统测试
│   └── test_mcp_tools.py          # MCP工具功能测试
│
└── 集成测试/
    ├── test_integration_simple.py # 简单集成测试（快速）
    └── test_integration_full.py   # 完整集成测试（全面）
```

## 🚀 快速开始

### 1. 运行所有测试

```bash
cd tests
python run_tests.py
```

### 2. 快速测试（推荐首次运行）

```bash
python run_tests.py --file test_integration_simple.py
```

### 3. 运行特定测试套件

```bash
# 只运行单元测试
python run_tests.py unit

# 只运行集成测试
python run_tests.py integration
```

### 4. 运行单个测试文件

```bash
python run_tests.py --file test_smart_cache.py
```

### 5. 列出所有可用测试

```bash
python run_tests.py --list
```

## 测试套件说明

### 单元测试 (Unit Tests)

#### `test_smart_logger.py` - 智能日志系统
- SmartLogger 创建和初始化
- 5个日志通道（system/trading/analysis/error/performance）
- 性能记录和统计
- 性能装饰器
- 全局单例模式

**测试数量**: 5个测试

#### `test_smart_cache.py` - 智能缓存系统
- 基本缓存操作（get/set）
- TTL过期机制
- 缓存未命中处理
- 缓存统计（命中率、键数）
- 缓存清除（全部/模式匹配）
- 缓存装饰器
- 全局单例模式

**测试数量**: 7个测试

#### `test_mcp_tools.py` - MCP工具功能
- MCP工具安全装饰器
- stdout隔离机制
- 环境变量辅助函数
- 通知开关逻辑
- 安全文件名生成

**测试数量**: 5个测试

### 集成测试 (Integration Tests)

#### `test_integration_simple.py` - 简单集成测试
快速验证核心功能，适合日常开发。

- 工具模块导入
- 日志和缓存集成
- 性能监控
- 异常处理
- 日志文件创建

**测试数量**: 5个测试  
**运行时间**: ~1秒

#### `test_integration_full.py` - 完整集成测试
全面测试所有优化功能的协同工作。

- P0-1: stdout隔离机制
- P0-2: 全局异常保护
- P0-3: 智能日志系统
- P1-1: 智能缓存系统
- 日志+缓存协同工作
- 错误日志+缓存协同
- 所有优化功能状态

**测试数量**: 7个测试  
**运行时间**: ~2秒

## 测试覆盖

### P0 - 稳定性功能
- stdout隔离 (防止污染MCP协议)
- 异常捕获 (防止MCP崩溃)
- 智能日志 (多通道 + 性能监控)

### P1 - 性能功能
- 智能缓存 (TTL + 统计)
- 性能监控 (慢查询检测)

### 集成功能
- 日志和缓存协同
- 错误处理和日志
- 性能装饰器

## 使用场景

### 开发时
```bash
# 快速验证改动
python run_tests.py --file test_integration_simple.py

# 验证特定模块
python run_tests.py --file test_smart_cache.py
```

### 提交前
```bash
# 运行所有单元测试
python run_tests.py unit
```

### 发布前
```bash
# 运行完整测试套件
python run_tests.py all
```

## ✅ 预期输出

### 成功示例
```
============================================================
🧪 简单集成测试
============================================================

📝 测试1: 工具模块导入
✅ 通过: 工具模块导入成功

📝 测试2: 日志和缓存集成
✅ 通过: 日志和缓存集成正常 (命中率: 66.7%)

...

============================================================
📊 测试结果: 5 通过, 0 失败
============================================================
```

### 失败示例
```
📝 测试3: 性能监控
❌ 失败: 'slow_function' not in stats

============================================================
📊 测试结果: 2 通过, 1 失败
============================================================
```

## 🔧 故障排查

### 导入错误
```bash
# 确保在项目根目录运行
cd d:/MCP
python tests/run_tests.py
```

### 权限错误
```bash
# 确保logs目录可写
mkdir logs
chmod 755 logs
```

### 依赖缺失
```bash
# 安装项目依赖
pip install -r requirements.txt
```

## 📝 添加新测试

### 1. 创建测试文件
```python
# tests/test_new_feature.py
import sys
import os

sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

def test_new_feature():
    """测试新功能"""
    print("\n📝 测试1: 新功能")
    
    try:
        # 测试代码
        assert True, "测试失败原因"
        
        print("✅ 通过: 新功能正常")
        return True
    except Exception as e:
        print(f"❌ 失败: {e}")
        return False

def run_all_tests():
    tests = [test_new_feature]
    passed = sum(1 for test in tests if test())
    failed = len(tests) - passed
    
    print(f"\n📊 测试结果: {passed} 通过, {failed} 失败")
    return failed == 0

if __name__ == "__main__":
    success = run_all_tests()
    sys.exit(0 if success else 1)
```

### 2. 添加到测试套件
编辑 `run_tests.py`，在 `TEST_SUITES` 中添加新测试文件。

## 💡 最佳实践

1. **测试隔离**: 每个测试应该独立，不依赖其他测试
2. **清理资源**: 使用临时目录，测试后清理
3. **明确断言**: 使用清晰的断言消息
4. **快速反馈**: 单元测试应该快速完成（<1秒）
5. **完整覆盖**: 集成测试验证实际使用场景

## 🔗 相关文档

- [系统化测试指南](../docs/测试/系统化测试指南.md) - 建议的验收顺序与测试矩阵
- [Claude/Windsurf 验收清单](../docs/客户端/Claude_Windsurf_验收清单.md) - 客户端导入与场景化验收
- [主 README](../README.md) - 项目总览

## 📞 支持

如果测试失败：
1. 查看错误信息和堆栈跟踪
2. 检查 `logs/` 目录下的日志文件
3. 运行 `python run_tests.py --file <失败的测试>` 单独调试
4. 查看相关模块的源代码

---

**最后更新**: 2024-12-16  
**测试覆盖率**: P0 (100%), P1 (100%)
